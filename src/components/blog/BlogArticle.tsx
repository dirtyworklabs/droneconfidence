    ...
    <table className="w-full min-w-[36rem] border-collapse text-left text-[0.92rem]">
      ...
      <tbody>
        {rows.map((row, rowIndex) => (
          <tr key={`${rowIndex}-${row[0] ?? ''}`} className="border-b border-ink/8 last:border-b-0">
            {row.map((cell, cellIndex) => (
              <td key={`${cellIndex}-${cell}`} className="px-4 py-3 align-top text-ink-soft">
                {cell}
              </td>
            ))}
          </tr>
...
export const BlogArticle = ({ post }: BlogArticleProps) => {
  ...
  return (
    <article>
      <Section tone="canvas" space="lg" className="overflow-hidden border-b border-ink/8">
        <Container width="text">
          <Reveal>
            <nav aria-label="Breadcrumb" className="mb-7 text-[0.86rem] text-ink-muted">
              <Link to={ROUTES.blog} className="transition-colors hover:text-eucalyptus">
                Guides
              </Link>
            ...
            <h1 className="mt-5 text-[clamp(2.25rem,7vw,4.5rem)] leading-[0.99]">{post.title}</h1>
        ...
        <Container width="text">
          <Reveal>
            <div className="rounded-[var(--radius-card)] border border-sage/20 bg-sage-soft/55 p-6 sm:p-8">
              ...
              <p className="mt-3 text-[1.04rem] leading-relaxed text-ink">{post.quickAnswer}</p>
            </div>
              ...
              <div>
                ...
                <ul className="mt-5 space-y-3">
                  {post.keyPoints.map((point) => (
                    <li key={point} className="flex gap-3 text-[0.98rem] leading-relaxed text-ink-soft">
              ...
              >
                ...
                <ol className="mt-4 space-y-2.5 text-[0.86rem] leading-snug text-ink-soft">
                  {headings.map((heading, index) => (
                    <li key={heading.id}>
                      <a href={`#${heading.id}`} className="transition-colors hover:text-eucalyptus">
          ...
          {post.sections.map((section) => {
            ...
            return (
              <Reveal key={section.heading}>
                <section
                  aria-labelledby={id}
                  className="border-b border-ink/8 py-8 first:pt-0 last:border-b-0 last:pb-0 sm:py-10"
                >
                  <h2 id={id} className="scroll-mt-28 text-[clamp(1.55rem,3vw,2rem)]">
                    {section.heading}
                  </h2>
                  ...
                  {section.callout ? (
                    <aside className="mt-7 rounded-[var(--radius-control)] border-l-2 border-sage bg-sage-soft/50 px-5 py-4 text-[0.97rem] leading-relaxed text-ink-soft">
                      {section.callout}
                    </aside>
                  ) : null}
                </section>
      ...
      <Section tone="sage" space="sm" aria-labelledby="article-cta-heading">
        <Container width="text">
          <Reveal>
            <Eyebrow>Practical help</Eyebrow>
            <h2 id="article-cta-heading" className="mt-4 text-[clamp(1.6rem,3vw,2.15rem)]">
              {post.cta.title}
            </h2>
      ...
      <Section tone="surface" space="sm" aria-labelledby="sources-heading">
        <Container width="text">
          <Reveal>
            <h2 id="sources-heading" className="text-[1.45rem]">Sources &amp; checking</h2>
            <p className="mt-3 text-[0.92rem] leading-relaxed text-ink-muted">
              This guide was checked against the sources below on {formatDate(post.reviewedAt)}.
              Rules, product features and local restrictions can change, so check the linked primary
              source when a decision depends on it.
            </p>
            <ul className="mt-5 space-y-4">
              {post.sources.map((source) => (
                <li key={source.url} className="min-w-0">
                  ...
                  <p className="mt-1 text-[0.8rem] leading-relaxed text-ink-muted">{source.role}</p>
                </li>
              ))}
            </ul>
            <p className="mt-6 text-[0.84rem] leading-relaxed text-ink-muted">
              General information only. Drone Confidence provides practical coaching, not legal advice,
              CASA certification or Remote Pilot Licence training.
            </p>
          </Reveal>
        </Container>
      </Section>

      <Section tone="canvas" space="sm" aria-labelledby="author-heading">
        <Container width="text">
          <Reveal>
            <div className="rounded-[var(--radius-card)] border border-ink/8 bg-surface p-6 sm:p-7">
              <Eyebrow>About the author</Eyebrow>
              <h2 id="author-heading" className="mt-4 text-[1.35rem]">{post.author}</h2>
              <p className="mt-3 text-[0.94rem] leading-relaxed text-ink-soft">
                Tom has worked with drones since 2013 across commercial, government and photography
                projects. Drone Confidence turns that experience into practical one-on-one help for
                everyday drone owners in Sydney.
              </p>
              <Link
                to={post.authorPath}
      ...
      {relatedPosts.length > 0 ? (
        <Section tone="canvas" space="md" aria-labelledby="related-guides-heading" className="border-t border-ink/8">
          <Container>
            <Reveal>
              <Eyebrow>Keep learning</Eyebrow>
              <h2 id="related-guides-heading" className="mt-4 text-[clamp(1.7rem,3.5vw,2.3rem)]">
                Related guides
              </h2>
            </Reveal>
