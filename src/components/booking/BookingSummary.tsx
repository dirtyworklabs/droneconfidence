...
import { formatShortDate, formatTimeRange } from '@shared/booking/format'
...
export const BookingSummary = ({
  ...
  const chosenTime =
    slot !== null && session !== null
      ? `${formatShortDate(new Date(slot), timeZone)}, ${formatTimeRange(
          new Date(slot),
          new Date(new Date(slot).getTime() + session.durationMinutes * 60000),
          timeZone,
        )}`
      : null
    ...
    <aside aria-labelledby="booking-summary-heading" className="lg:sticky lg:top-24">
      <div className="rounded-[var(--radius-card)] border border-ink/8 bg-surface p-5 sm:p-6">
        ...
        <dl className="mt-4 flex flex-col">
          ...
          <Row label="Date & time" value={chosenTime} />
        </dl>
