...
export const useBookingSelection = (): BookingSelectionState => {
  ...
  useEffect(() => {
    ...
    if (!sessionInvalid && !locationInvalid) return
  ...
  }, [params, setParams, rawSession, rawLocation, sessionId, locationId])
  ...
  const currentStep = session === null ? 1 : location === null ? 2 : 3

  return { session, location, currentStep, selectSession, selectLocation }
}
